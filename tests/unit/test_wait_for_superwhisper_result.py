...
def test_retries_when_transcription_done_but_no_llm_pass(recordings_dir):
    ...
    with (
        patch("time.sleep"),
        patch("pipeline.RECORDING_STABILITY_POLLS", 2),
        pytest.raises(TimeoutError, match="empty recording stub|no llmResult|stable"),
    ):
        wait_for_superwhisper_result("foo.m4a", since=since)
...
def test_does_not_fast_fail_while_llm_pass_is_in_flight(recordings_dir):
    ...
    with (
        ...
        patch("pipeline.RECORDING_STABILITY_POLLS", 2),
        patch("pipeline.SUPERWHISPER_TIMEOUT", 0.2),
        pytest.raises(TimeoutError, match="did not return"),
    ):
        wait_for_superwhisper_result("foo.m4a", since=since)
