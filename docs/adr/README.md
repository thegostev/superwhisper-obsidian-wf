...
- [0002 — launchd daemon for continuous transcription service](0002-launchd-daemon-pattern.md) ✅ **current** *(its recorded con, "no built-in health checks", is addressed by 0009 and 0010 — implemented)*
...
- [0009 — Heartbeat file as the transcriber liveness signal](0009-heartbeat-file-as-liveness-signal.md) ✅ **implemented**
- [0010 — Preflight wrapper and watchdog agent for launchd self-healing](0010-preflight-wrapper-and-watchdog-self-heal.md) ✅ **implemented**
...
> **Implementation status:** 0009/0010 and the spec are implemented (post-mortem 26-09-07 items #4–#10). The daemon writes a heartbeat, `health_check.py` reads it, the preflight wrapper guards the exec chain, and both plist templates live in `docs/launchd/`. See the spec's traceability table for requirement-by-requirement verification.
